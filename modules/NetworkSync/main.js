...
function copyAllFilesToTarget(ns, target) {
  ...
  if (files.length === 0) {
    ns.tprint("No allowed files found on home to copy.");
    return false;
  }
...
export async function main(ns) {
  ...
  if (allSuccess) {
    ns.tprint("Successfully synced all allowed files to all purchased servers.");
  } else {
    ns.tprint("Some servers failed to sync properly.");
  }
}
